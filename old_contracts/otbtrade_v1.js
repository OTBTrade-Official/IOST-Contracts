...
class Exchange {
        ...
        if (storage.mapHas(contractid, tokenSymbol)) {
            ...
            if (userOrders.length) {
                userOrders.forEach(o => {
                    storage.mapDel("orders", o);
                })
            }

            storage.mapPut(contractid, tokenSymbol, JSON.stringify([]));
        }
    }
                ...
                while (
                    ...
                    if (sOrder.amount <= amountNecessary) {
                        ...
                        this._transferToken(
                            ...
                            blockchain.contractName(),
                            sOrder.account,
                            (sOrderIOST * 1 - sellFee * 1).toFixed(8)
                        );
                        ...
                        this._transferToken(
                            ...
                            blockchain.contractName(),
                            order.account,
                            amountNecessary.toFixed(tokenDecimal)
                        );
    ...
    _feeHandler(user, yourRef, otherRef, yourFee, otherFee) {

        if (yourFee * 1 > 0) {

            if (yourRef !== null && yourRef !== "null") {
                // this._checkTransfer(user, "iost", user, yourRef, (yourFee * 0.25).toFixed(8), "User sends fee to reference")
                this._transferToken(
                    "iost",
                    blockchain.contractName(),
                    yourRef,
    ...
    _checkTransfer(user, tokenSymbol, from, to, amount, memo) {
        if (this._isLPContract(user)) {
            this._transferLP(user, tokenSymbol, to, amount, memo);
        }
        else {
            this._transferToken(tokenSymbol, from, to, amount);
        }
    }

    _transferLP(lpcontract, tokenSymbol, to, amount, memo) {
        let amt = new Float64(amount * 1 - 0.000000005).toFixed(8);
        let tcontract;
